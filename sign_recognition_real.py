...
def sign(imga, color, dop_area = False, mask = None):
    ...
    if target_color == 'red':
        for i in range(w):
            for j in range(h):
                if 'ros' in closest_colour(rgb_img[y+j][x+i]) or 'pink' in closest_colour(rgb_img[y+j][x+i]) or 'red' in closest_colour(rgb_img[y+j][x+i]) or 'tomato' in closest_colour(rgb_img[y+j][x+i]) or 'maroon' in closest_colour(rgb_img[y+j][x+i]) or 'brown' in closest_colour(rgb_img[y+j][x+i]) or 'gray' in closest_colour(rgb_img[y+j][x+i]) or 'black' in closest_colour(rgb_img[y+j][x+i]):
                    col+=1
        #print(col/(w*h))
        #print(w,h)
        if col/(w*h)>0.12 and h > 50:
            #show(rgb_img[y:y+h,x:x+w])
            x_coord = int((2*max_count.ravel()[0]+w)/2)
            y_coord = int((2*max_count.ravel()[1]+h)/2)
            sign_flag=True
...
def get_imgs_of_signs_real(test_img):
    list_of_signs = []
    imgs_of_signs = []
