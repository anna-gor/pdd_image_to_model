...
def get_graph(road_markings, imgs_of_signs):
    ...
    if sig and not (len(road_markings)%2==1 and road_markings[int(len(road_markings)/2)] in dividers) and road_markings[0] != 'прерывистая':
        ...
        R.add_edges_from(list(map(tuple, [stripe for stripe in new_obozn])), dirs='вперед') #list(map(tuple, [stripe for stripe in new_obozn])) дает таплы полос
        dirs_flg = True
    ...
    dirs = list(map(tuple, [stripe for stripe in new_obozn]))
    ...
    if road_markings[0]!='прерывистая':
        ...
        for i in range(len(stripes[0])):
            R.nodes[new_obozn[0][i]]["razm"] = line
            node_attribute_razm.append(new_obozn[0][i])
    ...
    #pos=nx.spring_layout(R)
    #nx.draw_networkx_edge_labels(R,pos,edge_labels=edge_dirs_labels)
    #nx.draw(R,pos, node_color=color_map, with_labels=True, node_size=1500)
    #pylab.show()
    ...
    if road_markings[0]!='прерывистая':
        k = 1
    for i in range(len(new_obozn)-1):
        for j in range(len(new_obozn[0])):
...
def draw_scheme_of_graph(R, imgs_of_signs):  
    ...
    def prepyvistaya_x(img, right = False, left = False):
        x1=x2=0
        if right:
            x1 = x2 = (int(img.shape[1])-1)
        if left:
            x1 = x2 = 0
